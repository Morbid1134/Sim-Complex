...
position.frame = function () {   
  ...
  // Reset all child nodes to empty strings
  for (var i = 0; i < position.childNodes.length; i++) {
    var a = position.childNodes[i];
    a.innerText = '';
  }

  // Calculate the starting index based on the position length
  var startIndex = (value.length > 1) ? 1 : 3;
  
  for (var i = 0; i < value.length; i++) {
    var index = startIndex + i;
    var a = position.childNodes[index];
    if (a) { // Ensure the node exists before setting its text
      a.innerText = value[i];
    }
  }
}
